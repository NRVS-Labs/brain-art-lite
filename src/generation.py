...
class NumpyArtGenerator:
    ...
    def __init__(self, resolution, feature_vector, num_layers, layer_width, activation_name, input_scalar_1=1.0, input_scalar_2=1.0, randomness=0.0):
        """Initialize the network.
            ...
            input_scalar_1      -- first input scalar value (between 0 and 1)
            input_scalar_2      -- second input scalar value (between 0 and 1)
            randomness          -- randomness factor (default: 0.0)
        """
        # Flags
        self.complete = False
        ...
        self.input_scalar_1 = input_scalar_1
        self.input_scalar_2 = input_scalar_2
        
        self.modify_from_metrics(feature_vector, self.input_scalar_1, self.input_scalar_2)
    ...
    def modify_from_metrics(self, feature_vector, input_scalar_1, input_scalar_2):
        """
        ...
        # Modify using input scalars
        width_modifier += int(input_scalar_1 * 10)
        layer_modifier += int(input_scalar_2 * 10)
    ...
    def forward_prop(self, inputs):
        ...
        for layer in range(0, self.num_layers):
            progress_msg = f"Current neural layer: {layer+1}/{self.num_layers}"
            print(progress_msg, end='\r')
            # st.write(progress_msg)
            st.toast(progress_msg)
            ...
            # Modify weights with input scalars to affect the color output
            W *= self.input_scalar_1 + 1
            bias = np.random.randn(W.shape[1]) * self.input_scalar_2

            activation_func = self.activation[1]
            results = activation_func(np.matmul(results, W) + bias)
    ...
    def print_details(self):
        ...
        Generator Settings:
            ...
            Hidden Layer Width: {self.layer_width},
            Activation Function: {self.activation[0]}
        """
        print(details)
        self.render_details(details)

    def render_details(self, details:str):
        st.info(details)
